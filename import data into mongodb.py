
##Importing data into Mongo db

import requests
import boto3
import json
import botocore
from pymongo import MongoClient
import requests
import json
import pandas as pd



AWSACCESSKEY="A***************"
AWSSECRETKEY="*******************"
AWSTOKEN=""

session = boto3.Session(profile_name='default')

# Taking the json file from s3 bucket

s3 = boto3.client("s3",aws_access_key_id=AWSACCESSKEY, aws_secret_access_key=AWSSECRETKEY, aws_session_token=AWSTOKEN, region_name='us-east-1')
#bucket = s3.Bucket('aws.project')
object = s3.get_object(Bucket='aws.storage.project',Key='bestbuy.json')
serializedObject = object['Body'].read().decode('utf-8')
file_data = json.loads(serializedObject)
#print(file_data)

#Mongodb data import

#All data collection

client = MongoClient('localhost', 27017)
db = client['bestbuydata_db']
best_buy_data = db['best_buy']
best_buy_data.insert_one(file_data)

best_buy_data = json_normalize(d['products'])
print(best_buy_data)
#Products collection
products = db['products']
records_ = df.to_dict(orient = 'records')
result = db.collection_products.insert_many(records_ )

print("Data imported")
client.close
